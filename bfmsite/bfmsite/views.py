...
from apps.customers.models import BFCustomerManager, BFCustomer, CustomerAnswer, SecurityQuestion
from apps.domains.models import Domain, BFName, Team, Account 
from apps.products.models import Product, CommonName, Occupation
from random import randint
from bfmsite.search import EAddress
...
def teamview(request, team, form_errors=None):
   try:
      bigfanteam = Team.objects.get(short_name=team)
      #request.session['bfteam'] = bigfanteam
   except:
      return redirect('http://www.bigfanmail.com')
      
  #request.session['domain_list'] = Domain.objects.filter(team=request.session['bfteam'])
   domain_list = Domain.objects.filter(team=bigfanteam)
   #return render(request, 'search/teamview.html', {'team':bigfanteam,'form_errors':form_errors})
   return render(request, 'search/teamview.html', {'team':bigfanteam, 'domain_list': domain_list,'form_errors':form_errors})
...
def bfname_usersearch(request, form_class=None):
    ...
    if not first_name and not last_name and not nickname and not occupation.get_eaddress():
       ...
       return render(request, 'search/teamview.html', {'team':request.POST['team'], 'form_errors':form_errors})
       #return render(request, 'search/teamview.html', {'team':request.POST['team'], 'domain_list':request.session['domain_list'], 'form_errors':form_errors})
    else:
       #domain     = request.POST['select_domain']
       finame     = EAddress(request.POST['first_name'].replace(' ',''))
       laname     = EAddress(request.POST['last_name'].replace(' ',''))
       team       = request.POST['team'].replace(' ','')
    ...
    for dom in doms:
       ...
       if users_in_domain.exists():
         ...
         if not (users_in_domain.filter(bfname=laname.get_eaddress()).exists()):
           if laname.get_eaddress():
              bfname_conf(laname, users_in_domain, domain)
...
def register_one(request): 
    qresult = BFCustomer.objects.get(email='aguschwan@gmail.com')
    if qresult:
        return "Duplicate"
    else:
        BFCustomer.objects.create_user(request.POST['username'], request.POST['email'], request.POST['first_name'], request.POST['last_name'], request.POST['password1'])
    return "Hello World"
